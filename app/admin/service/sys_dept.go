...
// SetDeptTree 设置组织数据
func (e *SysDept) SetDeptTree(c *dto.SysDeptGetPageReq, parentid int) (m []dto.DeptLabel, err error) {
...
func (e *SysDept) SetDeptPage(c *dto.SysDeptGetPageReq, parentid int) (m []models.SysDept, err error) {
	var list []models.SysDept
	err = e.getList(c, &list)
	for i := 0; i < len(list); i++ {
		if parentid == 0 { //所有
...
// Get 获取SysDept对象
func (e *SysDept) GetOrCreateById(id int, model *models.SysDept) error {
	if e.Orm == nil {
		e.Orm = getFirstOrm()
	}
	d := dto.SysDeptGetReq{}
	d.Id = id
	if e.Get(&d, model) != nil {
		di := dto.SysDeptInsertReq{}
		di.DeptId = id
		di.ParentId = 1 //根目录下
