...
const mongoCollectionDistanceV2 = "distancesv2"

type DistanceDataV2 struct {
	SvrTime     time.Time
	RadarID     int64 //雷达id
	TimeStamp   int64
	CommandCode int
...
func InitDistanceV2() error {
	...
	// 插入数据
	collection := client.Database(mongoRadarDBName).Collection(mongoCollectionDistanceV2)
	// 初始化时创建索引
...
func InsertDistanceDataV2(data *DistanceDataV2) error {
	data.SvrTime = time.Now()
	return insertDocumentData(mongoUri, mongoRadarDBName, mongoCollectionDistanceV2, data)
}

func GetLatestDistanceDataV2(devID int64) (*DistanceDataV2, error) {
	collection := client.Database(mongoRadarDBName).Collection(mongoCollectionDistanceV2)

	// 构建查询条件
	filter := bson.D{
		{Key: "radarid", Value: devID}, // 指定设备ID
	}
	// 设置查询选项：按时间降序排序，取第一条
	opts := options.FindOne().
		SetSort(bson.D{{Key: "svrtime", Value: -1}}) // -1 表示降序

	var result DistanceDataV2
