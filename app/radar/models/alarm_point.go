...
type AlarmPoint struct {
	...
	OrangeOption   string         `json:"orangeOption"   gorm:"size:255;comment:橙色预警条件"`                              //预警条件
	YellowOption   string         `json:"yellowOption"   gorm:"size:255;comment:黄色预警条件"`                              //预警条件
	BlueOption     string         `json:"blueOption"     gorm:"size:255;comment:蓝色预警条件"`                              //预警条件
	Interval       uint64         `json:"interval"       gorm:"comment:预警间隔时间（m）"`                                    //预警间隔时间（分）
	Duration       uint64         `json:"duration"       gorm:"comment:形变值的查询事件跨度（h）"`                                //形变值的查询事件跨度（h）
